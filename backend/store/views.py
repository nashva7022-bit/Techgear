...
from products.models import (BRAND_CHOICES, CASE_TYPE_CHOICES, Category,
                             Product, ProductVariant)

from .models import Cart, CartItem, Review, Wishlist, WishlistItem
...
def product_detail(request, slug):
    product = (
        Product.objects.filter(slug=slug, is_active=True, category__is_active=True)
        .select_related("category")
        .prefetch_related(
            "variants__images",
            "variants__device_model",
    ...
    ).exists()  # checks like selected variant is already in wish
    ...
    context = {
        ...
        "breadcrumbs": [  # shows nav path
            ...
            {
                "name": product.category.name if product.category else "Products",
                "url": None,
            },
...
def add_to_cart(request):
    ...
    if variant.stock <= 0:
        if is_ajax:
            return JsonResponse(
                {"ok": False, "error": "Sorry, this item is out of stock."}, status=409
            )
        ...
        if new_qty > 5:
            if is_ajax:
                return JsonResponse(
                    {
                        "ok": False,
                        "error": "Maximum 5 of the same item allowed in cart.",
                    },
                    status=409,
                )
...
def update_cart(request, item_id):
        ...
        if new_qty > cart_item.variant.stock:
            if is_ajax:
                return JsonResponse(
                    {
                        "ok": False,
                        "error": f"Only {cart_item.variant.stock} available in stock.",
                    },
                    status=409,
                )
            messages.error(
                request, f"Only {cart_item.variant.stock} available in stock."
            )
            return redirect("cart")
        if new_qty > 5:
            if is_ajax:
                return JsonResponse(
                    {"ok": False, "error": "Maximum 5 of the same item allowed."},
                    status=409,
                )
        ...
        if cart_item.quantity <= 1:
            if is_ajax:
                return JsonResponse(
                    {
                        "ok": False,
                        "error": "Minimum quantity is 1. Remove item to delete.",
                    },
                    status=400,
                )
            return redirect("cart")
    ...
    if is_ajax:
        ...
        customization_total = sum(i.customization_charge for i in all_items)
...
def toggle_wishlist(request, product_id):
    ...
    product = get_object_or_404(
        Product, pk=product_id, is_active=True, category__is_active=True
    )
    ...
    if not variant:
        if is_ajax:
            return JsonResponse(
                {"ok": False, "error": "No active variant found."}, status=404
            )
