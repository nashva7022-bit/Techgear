...
class Cart(models.Model):
    ...
    def total_price(self):
        return sum(item.subtotal for item in self.items.all() if item.is_available)

    @property
    def original_total(self):
        return sum(
            item.original_subtotal for item in self.items.all() if item.is_available
        )
    ...
    def customization_total(self):
        return sum(
            item.customization_charge for item in self.items.all() if item.is_available
        )
...
class CartItem(models.Model):
    ...
    def is_available(self):
        return (
            ...
            and self.variant.product.category.is_active
        )
    ...
    def primary_image(self):
        return (
            self.variant.images.filter(is_primary=True).first()
            or self.variant.images.first()
        )
