...
def admin_dashboard(request):
    ...
    from products.models import (BRAND_CHOICES,
                                 ProductVariant)
    ...
    wallet_liability = Wallet.objects.aggregate(total=Sum("balance"))[
        "total"
    ] or Decimal("0.00")
    ...
    best_brands_display = [
        {
            "brand": brand_map.get(
                b["variant__product__brand"], b["variant__product__brand"]
            ),
...
def user_list(request):
    ...
    if view_user_id:
        ...
        context["user_addresses"] = selected_user.addresses.all().order_by(
            "-is_default"
        )
...
def admin_wallet_credit(request, user_id):
    ...
    return render(
        request,
        "admin_panel/wallet_credit.html",
        {
            "wallet_user": user,
            "wallet": wallet,
        },
...
def admin_referral_list(request):
    ...
    usages = ReferralUsage.objects.select_related("referrer", "referred_user").order_by(
        "-created_at"
    )
    ...
    total_rewards_paid = (
        all_usages.filter(referrer_rewarded=True).aggregate(
            t=Sum("referrer_reward_amount")
        )["t"]
        or 0
    )
    total_signup_bonus = all_usages.aggregate(t=Sum("referred_reward_amount"))["t"] or 0
...
def admin_referral_settings(request):
    ...
    return render(
        request,
        "admin_panel/referral_settings.html",
        {
            "settings": settings_obj,
        },
    )
...
def admin_referral_mark_rewarded(request, usage_id):
    ...
    if not usage.referrer:
        messages.error(
            request, "The referrer account no longer exists — cannot pay the reward."
        )
        return redirect("admin_referral_list")
    ...
    try:
        with transaction.atomic():
            ...
            wallet.credit(
                ...
                reason=(
                    f"Referral reward (manual, by admin {request.user.email}) — "
