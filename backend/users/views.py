...
from products.models import BRAND_CHOICES, Category, Product

from .forms import (AddressForm, ChangeEmailForm, ChangePasswordForm,
                    EditProfileForm, SignupForm)
    ...
    if request.method == "POST" and form.is_valid():
        ...
        if attempts >= 10:
            ...
            return render(
                request,
                "auth/signup.html",
                {"form": form, "ref_code": request.GET.get("ref", "")},
                status=429,
            )
        ...
        try:
            ...
            if referral_code_input:
                request.session["pending_referral_code"] = (
                    referral_code_input  # stores temp
                )
            ...
            messages.error(
                request,
                "We couldn't send the code. Please check your email or try again.",
            )
            return redirect("signup")
            ...
            messages.info(
                request,
                "This email is already pending verification. Please check your inbox.",
            )
...
def verify_otp(request):
    ...
    otp_obj = (
        OTP.objects.filter(user=user, purpose="signup").order_by("-created_at").first()
    )
    ...
    if request.method == "POST":
        ...
        if not otp_obj or remaining_time <= 0:
            ...
            return render(
                ...
                {"remaining_time": 120, "user_email": user.email},
            )
        ...
        if len(otp) != 6:
            ...
            return render(
                request,
                "auth/verify_otp.html",
                {"remaining_time": remaining_time, "user_email": user.email},
                status=422,
            )
        ...
        if check_password(otp, otp_obj.otp):
            with transaction.atomic():
                ...
                if pending_referral:
                    from referrals.services import (apply_referral_on_signup,
                                                    validate_referral_code)

                    referral_code_obj, _ = validate_referral_code(
                        pending_referral, user.email
                    )
            ...
            messages.success(
                request, "Email verified! Please login with your password."
            )
            return redirect("login")
            ...
            if not user_check.is_verified:
                ...
                if not otp_exists:
                    generate_and_send_otp(
                        user_check, user_check.email, purpose="signup"
                    )
                    messages.warning(
                        request, "Account not verified. A new code has been sent."
                    )
                else:
...
def forgot_otp(request):
    ...
    otp_obj = (
        OTP.objects.filter(user=user, purpose="password_reset")
        .order_by("-created_at")
        .first()
    )
    ...
    if elapsed > OTP_EXPIRY_SECONDS:
        ...
        return render(
            request, "auth/forgot_otp.html", {"remaining_time": 0}, status=401
        )
...
def home(request):
    ...
    trending_products = (
        Product.objects.filter(
            is_active=True, is_trending=True, category__is_active=True
        )
        .prefetch_related("variants__images")
        .order_by("-created_at")[:4]
    )
...
def change_email_request(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print(
                f"DEBUG all OTPs for user: {list(all_otps.values('id', 'purpose', 'created_at'))}"
            )
...
def verify_email(request):
    ...
    otp_obj = (
        OTP.objects.filter(user=request.user, purpose="email_change")
        .order_by("-created_at")
        .first()
    )
    remaining_time = 0
    ...
    if request.method == "POST":
        ...
        if not otp_obj:
            messages.error(
                request, "No verification code found. Please request a new one."
            )
        ...
        if age_seconds > 60:
            ...
            messages.error(
                request, "Your code expired. A fresh one has been sent to your email."
            )
            return render(
                request,
                "profile/verify_email.html",
                {"email": new_email, "remaining_time": 60},
                status=401,
            )
        ...
        if otp_obj.attempts >= 5:
            otp_obj.delete()
            messages.error(
                request, "Too many incorrect attempts. Please request a new code."
            )
            return render(
                request,
                "profile/verify_email.html",
                {"email": new_email, "remaining_time": remaining_time},
                status=429,
            )
...
def resend_otp(request):
    ...
    if otp_obj and (timezone.now() - otp_obj.created_at).total_seconds() < 60:
        messages.warning(
            request, "Please wait 60 seconds before requesting a new code."
        )
...
def manage_addresses(request):
    ...
    return render(
        request,
        "profile/manage_addresses.html",
        {"addresses": addresses, "form": AddressForm()},
    )
...
def add_address(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "profile/manage_addresses.html",
            {"addresses": addresses, "form": form, "drawer_mode": "add"},
        )
    return redirect("manage_addresses")
