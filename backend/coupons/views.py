...
def coupon_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Coupon "{form.cleaned_data["code"]}" created successfully.')
            return redirect('coupons:coupon_list')
        return render(request, 'coupons/coupon_form.html', {
            'form': form, 'coupon': None, 'edit_mode': False,
        })
...
def coupon_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Coupon "{form.cleaned_data["code"]}" updated successfully.')
            return redirect('coupons:coupon_list')
        return render(request, 'coupons/coupon_form.html', {
            'form': form, 'coupon': coupon, 'edit_mode': True,
        })
