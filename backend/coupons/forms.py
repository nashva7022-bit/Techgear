    ...
    def clean_min_order_amount(self):
        ...
        if dtype == "fixed" and not moa:
            raise forms.ValidationError(
                "Minimum order amount is required for fixed discounts."
            )
    ...
    def clean(self):
        ...
        if dvalue is not None:
            if dvalue <= 0:
                ...
                self.add_error(
                    "discount_value", "Fixed discount cannot exceed ₹10,000."
                )
        ...
        if dtype == "percentage" and cap is None:
            self.add_error(
                "max_discount_cap",
                "Maximum discount cap is required for percentage discounts.",
            )
        if cap is not None:
            if dtype == "fixed":
                self.add_error(
                    "max_discount_cap", "Max cap only applies to percentage discounts."
                )
            elif cap <= 0:
                self.add_error(
                    "max_discount_cap", "Enter a valid amount greater than 0."
                )
            elif cap > 10000:
        ...
        if start and end:
            if end < start:
                ...
                self.add_error(
                    "end_date", "End date must be after start date, not the same day."
                )
