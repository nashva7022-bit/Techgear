...
def offer_list(request):
    product_offers = ProductOffer.objects.select_related("product").order_by(
        "-created_at"
    )
    category_offers = CategoryOffer.objects.select_related("category").order_by(
        "-created_at"
    )

    offers = sorted(
        chain(product_offers, category_offers), key=lambda x: x.created_at, reverse=True
    )
    ...
    product_page_obj = product_paginator.get_page(request.GET.get("product_page"))

    category_paginator = Paginator(category_offers, settings.OFFERS_PER_PAGE)
    category_page_obj = category_paginator.get_page(request.GET.get("category_page"))
...
def product_offer_create(request):
    ...
    form.fields["product"].queryset = Product.objects.filter(is_active=True).order_by(
        "name"
    )
    return render(request, "offers/product_offer_form.html", {"form": form})
...
def category_offer_create(request):
    ...
    form.fields["category"].queryset = Category.objects.filter(is_active=True).order_by(
        "name"
    )
    return render(request, "offers/category_offer_form.html", {"form": form})
