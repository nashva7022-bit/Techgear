...
from orders.models import VALID_TRANSITIONS, Order, OrderItem
from orders.services import (approve_return, cancel_order_item,
                             change_order_status, reject_return)
...
def order_detail(request, order_number):
    ...
    status_logs = order.status_logs.select_related(
        "changed_by"
    ).all()  # to get the admin
...
def change_status(request, order_number):
    ...
    if not new_status:
        if is_ajax:
            return JsonResponse(
                {"ok": False, "error": "No status provided."}, status=400
            )
...
def update_stock(request, variant_id):
    ...
    try:
        ...
        if is_ajax:
            return JsonResponse(
                {"ok": False, "error": "Invalid stock value."}, status=400
            )
        ...
        return JsonResponse(
            {
                ...
                "status": (
                    "out_of_stock"
                    if new_stock == 0
                    else ("low" if new_stock <= 10 else "ok")
                ),
            }
        )
...
def return_requests(request):
    ...
    items = (
        OrderItem.objects.filter(item_status="return_requested")
...
def admin_cancel_item(request, order_number, item_id):
    ...
    try:
        ...
        if is_ajax:
            return JsonResponse(
                {"ok": True, "message": f'"{item.product_name}" cancelled.'}
            )
...
def approve_return_view(request, order_number, item_id):
    ...
    try:
        ...
        messages.success(
            request,
            f'Return approved for "{item.product_name}". '
            f"Stock restored and refund credited to wallet.",
        )
        return redirect("admin_orders:order_detail", order_number=order_number)
