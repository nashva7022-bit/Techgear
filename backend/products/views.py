...
def category_edit(request, pk):
    ...
    return render(
        request,
        "admin_panel/category_edit.html",
        {
            "form": form,
            "category": category,
        },
...
def category_toggle_status(request, pk):
    ...
    if category.is_active:
        ...
        messages.success(
            request, f'"{category.name}" and its products have been deactivated.'
        )
    else:
        ...
        messages.success(
            request, f'"{category.name}" and its products have been activated.'
        )
    return redirect("admin_category_list")
...
def category_detail(request, pk):
    ...
    return render(
        request,
        "admin_panel/category_detail.html",
        {
            "category": category,
...
def _save_specs(product, post_data):
    ...
    for i, (name, value) in enumerate(zip(names, values)):
        ...
        if name and value:
            specs.append(
                ProductSpecification(product=product, name=name, value=value, order=i)
            )
    if specs:
...
def _handle_existing_variants(request, post_data, files, existing_variants):
    ...
    for variant in existing_variants:
        ...
        if variant.price is not None and variant.price <= 0:
            messages.error(
                request, f'Variant "{variant}": Price must be greater than 0.'
            )
            has_errors = True
            continue
...
def product_edit(request, pk):
    product = get_object_or_404(Product, pk=pk)
    existing_variants = (
        product.variants.prefetch_related("images").select_related("device_model").all()
    )
    ...
    return JsonResponse(
        {
            "success": True,
            "message": (
                "Variant activated." if variant.is_active else "Variant deactivated."
            ),
            "is_active": variant.is_active,
        }
    )
