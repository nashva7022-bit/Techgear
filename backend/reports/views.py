...
def sales_report(request):
    ...
    coupon_discount_total = orders_in_range.aggregate(r=Sum("coupon_discount"))[
        "r"
    ] or Decimal("0.00")
    total_discount_amount = orders_in_range.aggregate(r=Sum("discount_amount"))[
        "r"
    ] or Decimal("0.00")
    offer_discount_total = total_discount_amount - coupon_discount_total
    net_revenue = orders_in_range.aggregate(r=Sum("total_amount"))["r"] or Decimal(
        "0.00"
    )
...
def sales_report_excel(request):
    ...
    ws["A3"] = (
        ...
        f"to {data['end_date'].strftime('%d %b %Y')} "
    ...
    discount_sum_cell = ws.cell(
        row=total_row, column=5, value=f"=SUM(E{header_row+1}:E{total_row-1})"
    )
    discount_sum_cell.font = bold_font
    discount_sum_cell.number_format = currency_format
    amount_sum_cell = ws.cell(
        row=total_row, column=6, value=f"=SUM(F{header_row+1}:F{total_row-1})"
    )
    ...
    footer_cell = ws.cell(
        ...
        column=1,
        value=(
            f"Generated by {request.user.email} on "
